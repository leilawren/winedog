...
# You can append windows launch options to the exe
...
# CUSTOM WINE VERIONS
echo -e "${GREEN}Do you want to use a custom wine runner? [yes/no] ${RESET}"
read winechoice
if [ "$winechoice" = "yes" ]; then
    echo -e "${GREEN}please provide the directory to your wine runner:${RESET}"
    echo -e "${BLUE}example: /opt/wine-tkg/bin/wine${RESET}"
    read customwine
    echo "customwine=$customwine" >> runner.sh
elif [ "$winechoice" = "no" ]; then
    echo -e "${PINK}Okay! Moving on...${RESET}"
else
    echo -e "${RED}Invalid Input - Moving on...${RESET}"
fi

# GAMESCOPE
echo -e "${GREEN}Use gamescope? [yes/no]:${RESET}"
read gschoice
if [ "$gschoice" = "yes" ] && [ "$winechoice" = "yes" ]; then
    echo "gamescope -H 1080 -r 60 -f --force-grab-cursor -- $customwine $gamedir$binary"  >> runner.sh
elif [ "$gschoice" = "yes" ] && [ "$winechoice" = "no" ]; then
    echo "gamescope -H 1080 -r 60 -f --force-grab-cursor -- wine $gamedir$binary"  >> runner.sh
elif [ "$gschoice" = "no" ] && [ "$winechoice" = "yes" ]; then
    echo "$customwine $gamedir$binary"  >> runner.sh
elif [ "$gschoice" = "no" ] && [ "$winechoice" = "no" ]; then
    echo "wine $gamedir$binary" >> runner.sh
# write an else to deal with strange conditions I guess
fi

cat > winetricks.sh <<EOF
#!/bin/bash
WINETRICKS=$pfxdir winetricks -q
EOF
...
echo -e "${GREEN}Setup complete - Try launching your game from runner.sh${RESET}"
